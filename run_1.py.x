...
import logging
_logger = logging.getLogger("trading.scanner")
def get_safe_history(code, period="1mo"):  
    """具備錯誤處理、超時限制與狀態檢查的資料取得器"""
    ticker_str = f"{code}.TW" 
    try:
        ticker = yf.Ticker(ticker_str)
        
        # 強制加上 timeout 參數，避免網路卡死
        df = ticker.history(period=period, timeout=5, proxy=None, progress=False)
        
        # 💡 修正前：if df is empty or df.empty:
        # 💡 修正後：直接檢查是否為 None 或空 DataFrame 即可
        if df is None or df.empty:
            _logger.warning(f"⚠️ {code} 無價格資料 (可能已下市或停牌)")
            return None
            
        return df
    except Exception as e:
        _logger.error(f"❌ 讀取 {code} 失敗 (可能被限流或超時): {str(e)}")
        return None
        
#def get_safe_history(code, period="5d"):
#    """具備錯誤處理與狀態檢查的資料取得器"""
#    ticker_str = f"{code}.TW" # 根據你的市場代號調整
#    try:
#        ticker = yf.Ticker(ticker_str)
#        # 嘗試取得歷史資料
...
#        return df
#    except Exception as e:
#        _logger.error(f"❌ 讀取 {code} 失敗: {str(e)}")
#        return None
# --- 在你的掃描核心邏輯中 ---
def run_scan_for_list(candidate_list):
    valid_results = []
    for code in candidate_list:
        df = get_safe_history(code)
        
        # 只有當 get_safe_history 回傳有效資料時才進行後續分析
        if df is not None:
            # 這裡繼續執行你的 EMA / ADX / ATR 計算...
            # result = analyze_data(df)
            # valid_results.append(result)
            pass
        else:
            # 這裡可以選擇將該 code 加入一個「永久排除清單」
            # 下次掃描時直接跳過，省去等待 Yahoo 回傳錯誤的時間
            continue
...
class StockScanner:
    ...
    # ── 個股分析（策略通用） ────────────────────────────────────
    def analyze_one(self, code, capital, risk_pct, strategy="trend", name=""):
        code = str(code)
        try:
            # 💡 加密小技巧：隨機微幅延遲 0.1~0.5 秒，打散並行請求的時間點
            time.sleep(random.uniform(0.1, 0.5))
            df = get_safe_history(code, period="1mo")
            ...
            logger.warning("分析 %s 時發生錯誤，自動跳過: %s", code, e)
            return None    
    #def analyze_one(self, code, capital, risk_pct, strategy="trend", name=""):
    #    code = str(code)
    #    try:
    ...
    #        
    #        # 🟢 關鍵修正：如果資料為空，直接拋出錯誤並返回 None，不要讓掃描器掛掉
